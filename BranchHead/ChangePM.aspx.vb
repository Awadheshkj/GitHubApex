...
Partial Class RequestForprePnl
        ...
        Try
            ...
            If Not Page.IsPostBack Then
                ...
                If Len(Request.QueryString("bid")) > 0 Then
                    ...
                    Dim projectnamaget As String = ""
                    'checkProjectMang(hdnBrefID.Value)

                    If projectnamaget = "" Then
                        FillCampaignManager()
                        ddlProjectManager.SelectedItem.Text = checkProjectMang(hdnBrefID.Value)
    ...
    Private Sub FillCampaignManager()
        Try
            Dim sql As String = "Select UserDetailsID,(isnull(firstName,'')+' '+isnull(LastName,'')) as Name from APEX_UsersDetails where IsActive='Y' and IsDeleted='N' and Role not in('A','K','F','H') order by FirstName"
    ...
    Protected Sub btnSendRequert_Click(sender As Object, e As EventArgs) Handles btnSendRequert.Click
        Try
            ...
            notificationtransfer(hdnBrefID.Value, getLoggedUserID())
    ...
    Public Sub SendForApprovalBranchHead()
        Try
            ...
            sql &= "VALUES"
            sql &= "('Job Transfer'"
            sql &= ",'<b>Project Name: </b>" & Clean(getProjectName()) & "<br /><b>KAM: </b>" & getKAMName() & "<br /><hr  /><b>Message: </b>You have assign job'"
            sql &= ",'H'"
            ...
            sql &= ",'N'"
            sql &= ",'N') "
            sql &= " select NotificationID from APEX_Notifications where NotificationID= SCOPE_IDENTITY()"
            Dim ds As New DataSet
            ds = ExecuteDataSet(sql)
            'If ExecuteNonQuery(sql) > 0 Then
            If ds.Tables(0).Rows.Count > 0 Then
                InsertRecieptentDetails(ds.Tables(0).Rows(0)("NotificationID"))
                Dim projectnamaget As String = checkProjectMang(hdnBrefID.Value)
                Label1.Text = "Job has been successfully transferred to " & projectnamaget & "."
                'Label1.Text = "All Work Assign To " & projectnamaget & "."
                MessageDiv.Visible = True
                maindiv.Visible = False
    ...
    Private Sub InsertRecieptentDetails(ByVal NotificationID As Integer)
        Try
            Dim title As String = ""
            ...
            Dim sql As String = "Select NotificationID,NotificationTitle,NotificationMessage from APEX_Notifications where NotificationID=" & NotificationID & " and AssociateID=" & hdnBrefID.Value
            Dim ds As New DataSet
            ds = ExecuteDataSet(sql)
            If ds.Tables(0).Rows.Count > 0 Then
    ...
    Private Sub AssianProjectManager(p1 As String)
        Try
            ...
            If p1 <> "" Then
                ...
                sql &= " update  APEX_TaskAccount set InsertedBy='" & ddlProjectManager.SelectedValue & "' where RefjobcardID=" & capex.GetJobCardIDByBriefID(p1)
    ...
    Private Sub notificationtransfer(associateID As Object, userID As String)
        Try
            Dim sql As String = ""
            sql &= "update APEX_Recieptents set userID='" & ddlProjectManager.SelectedValue & "' where  userID=" & userID & " and refnotificationID in (select notificationID from APEX_Notifications where associateID=" & associateID & "))"
            sql &= " "
            ExecuteNonQuery(sql)
        Catch ex As Exception
