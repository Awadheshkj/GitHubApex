...
Partial Class RembursementClaim
    ...
    Private Sub InsertClaimTransaction()
        ...
        For Each grow In gvClaims.Rows
            ...
            Dim txtAmount, txthotel, txtnumberofdays, txtqty, txtrate, txtfrom, txtto, txtvendername, txtremarks As New TextBox
            'txtdate,
            ...
            If txtParticular.Text <> "" And txtAmount.Text <> "" Then
                If IsNumeric(txtAmount.Text) Then
                    If txtAmount.Text > 0 Then
                        ...
                        sql &= "(RefClaimID"
                        sql &= ",Description"
                        sql &= ",RequestedAmt,Amount, RefExpe_Category, Date, Hotel, Days, Qty, Rate, From_City, ToCity, VenderName, ExpensePeriodFrom, ExpensePeriodTo, ExpensePlace,Remarks)"
                        sql &= "VALUES"
                        sql &= "(" & hdnClaimMasterID.Value
    ...
    Public Function getclaimedtotalfORpm(ByVal jc As Integer) As Integer
        ...
        If jc > 0 Then
            ...
            sql += " where SubmittedUserID='" & getLoggedUserID() & "' and JobCardID='" & jc & "' and ta.AccountID='" & hdnTaskAccountID.Value & "'  and IsApproved ='Y'"
            sql += ""
            sql += ""
    ...
    Private Function checkEmptyGrid() As Boolean
        ...
        For Each grow In gvClaims.Rows
            ...
            If txtParticular.Text.Trim() <> "" And txtAmount.Text.Trim() <> "" And txtvandername.Text.Trim() <> "" Then
                i += 1
            End If
        Next
