...
Partial Class Estimate_VS_Actuals
            ...
            If Len(Request.QueryString("jid")) > 0 Then
                If Request.QueryString("jid") <> Nothing Then
                    If Request.QueryString("jid").ToString() <> "" Then
                        ...
                        jcno.Text = capex.GetJobCardByjcID(hdnJobCardID.Value)
                        ...
                        If hdnJobCardID.Value <> "" Then
                            ...
                            If role = "B" Or role = "A" Or role = "H" Then
                                GetEstimateID()
    ...
    Private Sub BindEstimateGrid()
        ...
        sqlGridString &= "select EFI.EstimateID,E.Category,E.Particulars,E.Quantity,E.Days,EFI.Actual as InvoiceAmt,E.Rate,"
        sqlGridString &= " EFI.AgencyFee,E.Remarks,EFI.Subtotal,EFI.Managementfee,EFI.total,EFI.servicetax,EFI.GrandTotal  from [dbo].[Apex_Estimate_for_invoicing] EFI"
        sqlGridString &= " inner join APEX_tempEstimate E on EFI.RefTempEstimateID =E.EstimateID  "
        sqlGridString &= " where E.refbriefID=" & hdnBrief.Value & " and EFI.EstimateID between " & Request.QueryString("min") & " and " & Request.QueryString("max") & ""
        sqlGridString &= ""
        ...
        gdvEstimate.DataBind()
        If ds.Tables(0).Rows.Count > 0 Then
            lblSubTotal.Text = ds.Tables(0).Rows(0)("Subtotal")
            lblMangnmtFees.Text = ds.Tables(0).Rows(0)("Managementfee")
            lblTotal.Text = ds.Tables(0).Rows(0)("total")
            lblServiceTax.Text = ds.Tables(0).Rows(0)("servicetax")
            lblGrandTotal.Text = ds.Tables(0).Rows(0)("GrandTotal")
        End If
    ...
    Private Sub Updatejobcard()
        Dim sql As String = "update APEX_JobCard set IsEstimateVsActuals='Y' where JobcardID = " & hdnJobCardID.Value
        ExecuteNonQuery(sql)
    End Sub
