...
Partial Class OpenEstimate
    ...
    Public Sub SendForApprovalBranchHead(ByVal type As String, ByVal bid As String, ByVal desig As String, Optional stg As String = "other")
        ...
        If type = 43 Then
            sql &= "('Prepnl Rejected'"
            'sql &= ",'Your Estimate Rejected'"
            sql &= ",'<b>Project Name: </b>" & getProjectName() & "<br /><b>KAM: </b>" & getKAMName() & "<br /><hr  /><b>Message: </b>Your PrePnl Rejected from " & getLoggedUserName() & "'"
        Else
            sql &= "('PrePnl Approval'"
            'sql &= ",'Approval for the Estimate'"
            'The prepnl increase request has been accepted by branch head (NME) and has automatically been approved for PM at your end. 
            sql &= ",'<b>Project Name: </b>" & getProjectName() & "<br /><b>KAM: </b>" & getKAMName() & "<br /><hr  /><b>Message: </b> The prepnl increase request has been accepted by branch head (" & getLoggedUserName() & ") and has automatically been approved for PM at your end.'"
        End If
    ...
    Protected Sub btnApproval_Click(sender As Object, e As EventArgs) Handles btnApproval.Click
        ...
        If ExecuteNonQuery(sql) > 0 Then
            ...
            lblMsg.Text = "Pre PNL approved successfully."
            divError.Visible = False
            MessageDiv.Visible = True
            SendForApprovalBranchHead(clsApex.NotificationType.PrepnlapprovalfromBHtoKAM_AfterJC, hdnBrief.Value, 2, "Estimate")

        End If
    End Sub

    Protected Sub btnsendapproval_Click(ByVal approved As String)
        ...
        capex.UpdateRecieptentExecuted(hdnNodificationID.Value, getLoggedUserID())
        If approved = "R" Then
            SendForrejectBranchHeadPrepnl(clsApex.NotificationType.approveprepnlAmtsuccess, hdnBrief.Value, 3, "KAM Updates On Additional cost Request")
        Else
            SendForApprovalBranchHeadPrepnl(clsApex.NotificationType.approveprepnlAmtsuccess, hdnBrief.Value, 3, "KAM Updates On Additional cost Request")
        End If
    ...
    Public Sub SendForrejectBranchHeadPrepnl(ByVal type As String, ByVal bid As String, ByVal desig As String, ByVal approvalstr As String, Optional stg As String = "PrepnlApproval")
        Try
            Dim sql As String = "INSERT INTO APEX_Notifications"
            sql &= "(NotificationTitle"
            sql &= ",NotificationMessage"
            ...
            sql &= "VALUES"
            sql &= "('" & approvalstr & "'"
            sql &= ",'<b>Project Name: </b>" & getProjectName() & "<br /><b>KAM: </b>" & getKAMName() & "<br /><hr  /><b>Message: </b>Approve Pre P&L Amount Rejected'"
            sql &= ",'H'"
            sql &= "," & type
            sql &= "," & bid
            sql &= ",getdate()"
    ...
    Private Sub InsertRecieptentDetailsPrepnl(ByVal type As String, ByRef bid As String, ByVal deg As String, Optional stag As String = "other")
        Try
            ...
            If stag <> "other" Then
                ...
                sql3 = "select projectManagerID as insertedBy from APEX_JobCard where refBriefID= " & hdnBrief.Value
            Else
                sql3 = "Select UserDetailsID from APEX_UsersDetails where Designation=" & deg & ""
            End If
    ...
    Private Sub InsertRecieptentDetails()
        Dim message As String = ""
        Dim notificationid As String = ""
        Dim sql As String = "Select NotificationID,NotificationTitle,NotificationMessage from APEX_Notifications where Type=" & NotificationType.PPNLApproved & " and AssociateID=" & hdnBrief.Value & " order by NotificationID desc"
        Dim ds As New DataSet
        ds = ExecuteDataSet(sql)
        If ds.Tables(0).Rows.Count > 0 Then
            If Not IsDBNull(ds.Tables(0).Rows(0)("NotificationID")) Then
    ...
    Protected Sub btnRejected_Click(sender As Object, e As EventArgs) Handles btnRejected.Click
        ...
        If ExecuteNonQuery(sql) > 0 Then
            btnsendapproval_Click("R")
            trAppButtons.Visible = False
            lblMsg.Text = "Pre PNL is rejected."
            divError.Visible = False
            MessageDiv.Visible = True
            SendForApprovalBranchHead(clsApex.NotificationType.PrepnlRejectfromBHtoKAM_AfterJC, hdnBrief.Value, 2, "Estimate")
    ...
    Private Sub BindGrid()
        Try
            ...
            sql &= " from Apex_PrePnLCost where RefBriefID=" & hdnBrief.Value & " "
            ...
            sql &= "  union "
            sql &= "  Select ROW_NUMBER() OVER(ORDER BY PrePnLCostID DESC) AS Row,*,0,0,0,0,0,'' from Apex_tempPrePnLCost where RefBriefID=" & hdnBrief.Value & "  order by PrePnLCostID Asc"
            ...
            Else
                ...
                btnApproval.Text = "Already taken action"
            End If
    ...
    Private Sub btnApproval_Command(sender As Object, e As CommandEventArgs) Handles btnApproval.Command
