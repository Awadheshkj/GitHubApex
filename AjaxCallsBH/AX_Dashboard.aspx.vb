...
Partial Class AjaxCalls_Dashboard
    ...
    Private Function FillJobCounts() As String
        ...
        Sql = "select count(1)TotalJobs from Apex_Leads where primaryActivityID='" & role & "'"
        Sql &= " or RefActivityTypeID Like '%'+(select ProjectType from [dbo].[APEX_ActivityType] where projectTypeID='" & role & "')+'%' "
        ...
        Sql &= " where jobcardno  is not null and JobCompleted<>'Y' and B.primaryActivityID='" & role & "'"
        Sql &= " or jc.RefActivityID Like '%'+(select ProjectType from [dbo].[APEX_ActivityType] where projectTypeID='" & role & "')+'%' "
        'If Not (role = "H" Or role = "F") Then
        '    Sql &= " and B.insertedby='" & uid & "'"
        'End If
        Sql &= " select count(1)CompletedJobs from Apex_jobcard jc"
        Sql &= " inner join Apex_brief B on jc.RefbriefID=B.briefID"
        Sql &= " where JobCompleted='Y' and jc.primaryActivityID='" & role & "' "

        'If Not (role = "H" Or role = "F") Then
        '    Sql &= " and B.insertedby='" & uid & "'"
        'End If

        Sql &= " select count(1)PipelineJobs from Apex_Leads where ISbriefed='N' and primaryActivityID='" & role & "' "
        'If Not (role = "H" Or role = "F") Then
