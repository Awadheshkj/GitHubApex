	...
	$.fn.advancedtable = function(custom) {
		...
		function redrawTable() {
			...
			if(tableRows > settings.rowsPerPage){
				...
				if(true){
					...
					for(var i = 0;i < pages; i++){
					    blockNavigation += '<li' + ((settings.currentPage == i) ? ' class="active"' : "") + '><a href="javascript:returnblank();">' + (i + 1) + '</a></li>';
					}
					blockNavigation += '</ul>';
				}else{
			...
			if(settings.sorting){
				...
				//		$(this).html('<a href="#" id="sorthandle' + sorthandle + '">' + $(this).html() + '</a><span id="sortaschandle' + sorthandle + '" class="sortshowhandle"><img src="' + settings.ascImage + '" alt=""/></span><span id="sortdeschandle' + sorthandle + '" class="sortshowhandle"><img src="' + settings.descImage + '" alt=""/></span><span id="sorttypehandle" class="allsorttypehandle" style="display:none"></span>');
