...
Partial Class OpenPrePnLManager
    ...
    Private Sub BindGrid()
        Try
            ...
            sql &= "  Select ROW_NUMBER() OVER(ORDER BY PrePnLCostID DESC) AS Row,*,0,'',0 from Apex_tempPrePnLCost where RefBriefID=" & hdnBriefID.Value & " "
        ...
        Try
            If e.CommandName = "Add" Then
                ...
                If ExecuteNonQuery(sql) > 0 Then
                    ...
                    Dim sql2 As String = ""
                    sql2 &= " insert into APEX_PrePnlCostTrans(PrepnlcostID,PrvEventTotalPRV,PrevEventTotalCUR,REquestAmt,RequestDT,ApprovedBy,ApprovedStatus)"
                    sql2 &= " values( (select max(prepnlcostid) from APEX_PrePnLCost) ,0," & Clean(txtPreEventCost.Text) & "," & REquestAmt & ",getdate(),'0','N')"
                    sql2 &= ""
                    sql2 &= ""
                    sql2 &= ""
    ...
    Protected Sub btnsendapproval_Click(sender As Object, e As EventArgs) Handles btnsendapproval.Click
        ...
        For i As Integer = 0 To gvPrePnLCost.Rows.Count - 1
            ...
            Dim sql As String = ""
            sql &= "Update APEX_PrePnlCostTrans set SendReq='Y' where ID=" & btn.CommandArgument & ""
            sql &= ""
            sql &= ""
