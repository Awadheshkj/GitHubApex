...
Partial Class Estimate_VS_Actuals
                ...
                If Request.QueryString("jid") <> Nothing Then
                    If Request.QueryString("jid").ToString() <> "" Then
                        ...
                        If hdnJobCardID.Value <> "" Then
                            ...
                            If role = "K" Or role = "A" Or role = "H" Or role = "F" Then
                                GetEstimateID()
                                If role = "F" Then
            ...
            If hdnJobCardID.Value <> "" Then
                ...
                If role = "K" Then
                    If minmumCCprofitpercentage() > percentage Then
                        ...
                        btnsendrequesttobranchhead.Visible = True
                        btnrejectfromBH.Visible = False
    ...
    Private Sub getservicetax(BriefID As String)
        Try
            ...
            Dim sql As String = "Select top 1 cast(round((EstimatedServiceTax/EstimatedTotal)*100,2) as Numeric(18,2))servicetax from  [dbo].[APEX_Estimate] where refbriefID='" & BriefID & "'"
    ...
    Public Sub SendForApprovalBranchHead(ByVal type As String, ByVal bid As String, ByVal desig As String, Optional stg As String = "other")
        ...
        sql &= "VALUES"
        If type = 10 Then
            sql &= "('Final Estimate Rejected'"
            'sql &= ",'Your Estimate Rejected'"
            sql &= ",'<b>Project Name: </b>" & getProjectName() & "<br /><b>KAM: </b>" & getKAMName() & "<br /><hr  /><b>Message: </b>Your Final Estimate Rejected'"
        Else
    ...
    Private Sub getEstimatevsprepnlHistory()
        ...
        ds = ExecuteDataSet(sql)
        If ds.Tables(0).Rows.Count > 0 Then
            tblestimatehistory.Visible = False
            gridfinalesimate.DataSource = ds
            gridfinalesimate.DataBind()
        Else
            tblestimatehistory.Visible = True
        End If
            ...
            If hdnJobCardID.Value <> "" Then
                ...
                If role = "K" Then
                    If minmumCCprofitpercentage() > percentage Then
                        ...
                        lblwarning.Text = "Your profit percentage(" & percentage & ") is below the approved percentage(" & minmumCCprofitpercentage() & "). Waiting approval from branch head."
                        Updatejobcard("P")
                        UpdateReason(hdnJobCardID.Value, txtreason.Text, "KAM")
                        SendForApprovalToBranchHead(clsApex.NotificationType.finalestimateapproval, hdnJobCardID.Value, 6)
                        btnAdd.Visible = False
                    Else
                        Updatejobcard("Y")
    ...
    End Sub
    Private Sub UpdateReason(ByVal refjobcardID As Integer, ByVal reason As String, ByVal reasonby As String)
        Dim sql As String = ""
        sql &= "  INSERT INTO [dbo].[Apex_FinalEstimateReasonHistory]"
        sql &= "          ([Reason]           ,[ReasonFrom]           ,[RefjobcardID])"
        sql &= "    VALUES           ('" & Clean(reason) & "','" & Clean(reasonby) & "', '" & refjobcardID & "')"
        ExecuteNonQuery(sql)
    ...
    Private Sub btnRejected_Click(sender As Object, e As EventArgs) Handles btnRejected.Click
        Messagebranchhead.Visible = True
        divpnl.Visible = False
        lblreasonmsg.Text = "Please enter rejection reason"
        btnsendrequesttobranchhead.Visible = False
        btnrejectfromBH.Visible = True

    End Sub

    Private Sub GetReasons()
        Dim sql As String = ""
        sql &= " select  Reason, ReasonFrom, InsertedOn, RefjobcardID from [dbo].[Apex_FinalEstimateReasonHistory]"
        sql &= "  Where RefjobcardID='" & hdnJobCardID.Value & "'"
        sql &= "  order by InsertedOn desc"
        Dim ds As New DataSet
        ds = ExecuteDataSet(sql)
        If ds.Tables(0).Rows.Count > 0 Then
            gdvfinalestimateReason.DataSource = ds
            gdvfinalestimateReason.DataBind()
        End If
    End Sub

    Private Sub btnrejectfromBH_Click(sender As Object, e As EventArgs) Handles btnrejectfromBH.Click
        Dim sql As String = ""
        sql &= " update [dbo].[APEX_JobCard] set  isestimatevsactuals=null,isinvoiceing=null where  jobcardID=" & hdnJobCardID.Value
        ExecuteNonQuery(sql)
        MessageDiv.Visible = True
        lblMsg.Text = "Final Estimate Rejected Successfully."
        divpnl.Visible = False
        Dim capex As New clsApex
        If Len(Request.QueryString("nid")) > 0 Then
            If Request.QueryString("nid") <> Nothing Then
                ...
                btnCancelApp.Visible = True
                hdnNodificationID.Value = Request.QueryString("nid")
                UpdateReason(hdnJobCardID.Value, txtreason.Text, "BH")
                capex.UpdateRecieptentExecuted(hdnNodificationID.Value, getLoggedUserID())
                SendForApprovalBranchHeadToKamReject(clsApex.NotificationType.finalestimateapprovalReject, hdnJobCardID.Value, 2, "Reject")
            End If
        End If
    End Sub

    Public Sub SendForApprovalBranchHeadToKamReject(ByVal type As String, ByVal bid As String, ByVal desig As String, Optional stg As String = "other")
        Dim sql As String = "INSERT INTO APEX_Notifications"
        sql &= "(NotificationTitle"
        sql &= ",NotificationMessage"
        sql &= ",Severity"
