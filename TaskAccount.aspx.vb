...
Partial Class TaskAccount
    ...
    Private Sub BindgdvAccount()
        ...
        sqlacString &= " AccountID, Particulars, Quantity, Amount, ta.Total,ta.Description,Ta.TaskName,Ta.City"
        ...
        sqlacString &= "	where CM.refTaskID=TA.accountID and isapproved='Y'),0)Claimed,notificationType"
        sqlacString &= " ,IsNull((case when EA.finaceApproval<>'Y' or EA.BranchHeadApproval<>'Y' then 0 else EA.AdvanceAmtRequest end),0)empadv"
        sqlacString &= " ,IsNull((case when VA.finaceApproval<>'Y' or VA.BranchHeadApproval<>'Y' then 0 else VA.AdvanceAmtRequest end),0)vendadv"
        ...
        sqlacString &= " left join Apex_EmployeeAdvanceExpenses EAE on ta.AccountID=EAE.ReftaskID left join Apex_EmployeesAdvence EA On EAE.RefEmpAdvanceID=EA.ID"
        sqlacString &= " left join Apex_VendorAdvanceExpenses VAE on ta.AccountID=VAE.ReftaskID left join Apex_VendorAdvence VA On VAE.RefEmpAdvanceID=VA.ID"
    ...
    Protected Sub gvPrePnLCost_RowEditing(sender As Object, e As GridViewEditEventArgs) Handles gdvAccount.RowEditing
        ...
        Dim ddlnotifictaionType As DropDownList = CType(gdvAccount.Rows(index).FindControl("gv_ddlnotificationType"), DropDownList)
        ...
        Dim sqlta As String = " select category,TL,notificationType from APEX_TaskAccount where AccountID =" & hdnTaskAccountID.Value & ""
        ...
        If dsta.Tables(0).Rows.Count > 0 Then
            ...
            ddlnotifictaionType.SelectedValue = dsta.Tables(0).Rows(0)("notificationType")
        End If
    ...
    Protected Sub gdvAccount_RowUpdating(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewUpdateEventArgs) Handles gdvAccount.RowUpdating
        ...
        Dim ddlnotificationType As New DropDownList
        ...
        ddlnotificationType = CType(gdvAccount.Rows(e.RowIndex).FindControl("gv_ddlnotificationType"), DropDownList)
        ...
        If CheckTotalAmount(total) = True Then
            UpdateRecord(d1, d2, d3, d4, d5, startdate.Text, Enddate.Text, Description.Text, category.SelectedValue, TL.SelectedValue, Clean(txtcity.Text), Clean(txttaskname.Text), ddlnotificationType.SelectedValue)
            '    BindBalance()
            CalculateBalance()
            Getprepnlwithcategory()
        Else
    ...
    Private Sub UpdateRecord(ByVal TaskAccountID As String, ByVal Prticulars As String, ByVal Quantity As String, ByVal Amount As String, ByVal total As String, ByVal Startdate As String, ByVal Enddate As String, ByVal Description As String, ByVal Category As String, ByVal TL As String, ByVal city As String, ByVal taskname As String, ByVal ddlnotificationType As String)
        ...
        sql &= " ,City='" & Clean(city) & "'"
        sql &= " ,notificationType='" & Clean(ddlnotificationType) & "'"
    ...
    Protected Sub gdvAccount_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gdvAccount.RowCommand
        If e.CommandName = "add" Then
            ...
            ddlnotificationType = CType(frow.FindControl("ddlnotificationType"), DropDownList)
            ...
            If CheckTotalAmount(total) = True Then
                ...
                End If
                sqlString &= "           ,[Balance]"
                sqlString &= "           ,[Total],startdate,Enddate,Description,InsertedBy,City,TaskName,notificationType)"
                sqlString &= "     VALUES"
                ...
                sqlString &= "           ,'" & Clean(ddlnotificationType.SelectedValue) & "'"
    ...
    Protected Sub gdvAccount_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gdvAccount.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.Footer Then
            ...
            Dim sql As String = "Select UserDetailsID,(isnull(firstName,'')+' '+isnull(LastName,'')) as Name from APEX_UsersDetails where IsActive='Y' and IsDeleted='N' and Role not in('A','F','H','K') order by LTRIM(FirstName)"
            Dim ds As New DataSet
            ds = ExecuteDataSet(sql)
            If ds.Tables(0).Rows.Count > 0 Then
                ddlLead.DataSource = ds.Tables(0)
