...
Partial Class AjaxCalls_PMDetails
        ...
        If Len(Request.QueryString("call")) > 0 Then
            If Request.QueryString("call").ToString() <> "" Then
                ...
                ElseIf callid = "22" Then
                    ...
                    'For JC Summary on PM dashboard
                ElseIf callid = "23" Then
                    result = fillJCsummary(jid)
                ElseIf callid = "24" Then
                    result = fillJCsummaryForEstimate(jid)
                ElseIf callid = "25" Then
                    result = FillNotification()
    ...
    '    sql &= " "
    '    sql &= "  Select Distinct Briefname,jc.ModifiedOn,(select (FirstName + ' ' + Isnull(LastName,'')) from APex_usersdetails where userdetailsID=b.InsertedBy)Kamname,b.BriefID,Client, "
    '    sql &= " link =case when ProjectManagerID=" & getLoggedUserID() & "  then (case when type =7 And rec.Isseen='N' and (jc.JobCompleted ='N' or jc.JobCompleted is null)  "
    ...
    '    sql &= "	'&kid=y' else 'PrePnlManager.aspx?mode=edit&bid=' + convert(varchar(10),BriefID) + '&kid=y' end) else 'BriefManagerview.aspx?mode=edit&bid=' +  	   convert(varchar(10),BriefID) + 	'&kid=y' end,  	     "
    '    sql &= "   jc.jobCardID,isnull(jc.JobcardNo,'N/A')JobcardNo,Estimate.EstimatedTotal as Budget,    isnull((select sum(cl.Amount) from APEX_ClaimMaster as cm 		      "
    ...
    '    sql &= " ,(case when ProjectManagerID = " & getLoggedUserID() & " then 'Y' else 'N' end)ismyproject"
    '    sql &= "  from  APEX_JobCard as jc   	  						"
    '    sql &= "			   Inner join APEX_Notifications as nt on jc.RefBriefID=nt.AssociateID   	  							   "
    '    sql &= "			   left outer join APEX_ActivityType  as At on Jc.PrimaryActivityID=At.ProjectTypeID   	  							    "
    '    sql &= "			   join dbo.APEX_Recieptents as rec on nt.notificationID=rec.refnotificationID   	  							   "
    ...
    '    sql &= "			    where ( ProjectManagerID = " & getLoggedUserID() & " or ProjectManagerID in(" & capex.getuserdetailsID(getLoggedUserID()) & ") or  TA.TL in(" & capex.getuserdetailsID(getLoggedUserID()) & ") or TA.TL=" & getLoggedUserID() & ") and jc.isactive='Y'  order by jc.ModifiedOn Desc "
    '    sql &= " "
    '    sql &= ""
    '    sql &= ""
    '    sql &= " "
    ...
    Private Function FillDetailsPM1() As String
        ...
        sql &= " "
        sql &= "   Select Distinct Briefname,jc.ModifiedOn,JobCompleted,"
        sql &= "   (select (FirstName + ' ' + Isnull(LastName,'')) from APex_usersdetails where userdetailsID=b.InsertedBy)Kamname,"
        sql &= "   b.BriefID,Client,  link =case when ProjectManagerID=" & getLoggedUserID() & "  then "
        sql &= "   (case when type =7 And rec.Isseen='N' and (jc.JobCompleted ='N' or jc.JobCompleted is null)    "
        sql &= "    then 'PrePnlManager.aspx?mode=edit&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID)  "
        sql &= "	 + '&kid=y'    	when jc.isPrePnL='Y' and (jc.JobCompleted ='N' or jc.JobCompleted is null) then "
        sql &= "	 'OpenPrePnlManager.aspx?mode=edit&bid=' +  	   convert(varchar(10),BriefID) + 	'&kid=y' else "
        sql &= "	 'PrePnlManager.aspx?mode=edit&bid=' + convert(varchar(10),BriefID) + '&kid=y' end) else "
        sql &= "	 'BriefManagerview.aspx?mode=edit&bid=' +  	   convert(varchar(10),BriefID) + 	'&kid=y' end,  	        "
        sql &= "	 jc.jobCardID,isnull(jc.JobcardNo,'N/A')JobcardNo,"
        sql &= "	 Budget=convert(varchar(50),Estimate.EstimatedTotal) ,FinalEstimate=   "
        sql &= "	 convert(varchar(50),(Select top 1 total from Apex_TempEstimate where refbriefID in (jc.RefBriefID))),"
        ...
        sql &= "	 where jobcardNo=jc.jobCardID and isapproved='Y'),0.00)as Claims, 			    	 "
        sql &= "	 Final_Cost="
        sql &= "	  convert(varchar(50),(Select Sum(preeventcost)pre from [APEX_prePnLCost] where refbriefID in (b.BriefID))) ,postpnl= "
        sql &= "	  convert(varchar(50),isnull((select Sum(posteventcost) from [dbo].[APEX_tempPostPnLCost] where refbriefid=b.BriefID),0.00))"
        sql &= "	  ,"
        sql &= "	 Savings=isnull((Estimate.EstimatedTotal-(select Sum(posteventcost) from [dbo].[APEX_tempPostPnLCost] "
        sql &= "	 where refbriefid=b.BriefID)),0.00), 	  JobStatus = case when (JobCompleted='N' or JobCompleted is null) and "
        sql &= "	 JobCardNo is null then '-' 						      "
        sql &= "	 when (JobCompleted='N' or JobCompleted='P' or JobCompleted is null) and JobCardNo is not null then 'Open' 	"
        sql &= "	  when (JobCompleted='Y' or JobCompleted is null) and JobCardNo is not null then 'Close'	"
        sql &= "	  end,jc.INsertedOn,Isnull(Convert(varchar(20),jc.ModifiedOn,106),'N/A')ModifiedOnDt,ProjectManagerID  	  ,"
        sql &= "	 (select (FirstName + ' ' + Isnull(LastName,'')) from APex_usersdetails where userdetailsID=ProjectManagerID)ProjectManager,"
        sql &= "	 isnull(refprepnlID,0)refprepnlID ,(case when ProjectManagerID = " & getLoggedUserID() & " then 'Y' else 'N' end)ismyproject  from  APEX_JobCard as jc"
        sql &= "	Inner join APEX_Notifications as nt on jc.RefBriefID=nt.AssociateID  "
        sql &= "	left outer join APEX_ActivityType  as At on Jc.PrimaryActivityID=At.ProjectTypeID   "
        sql &= "	join dbo.APEX_Recieptents as rec on nt.notificationID=rec.refnotificationID"
        sql &= "	inner join APEX_Brief as b on jc.RefBriefID = b.BriefID   	  							   			   "
    ...
    Private Function FillDetailsPM1PREPNL(ByVal jc As String) As String
        ...
        sql &= "	'&kid=y' else 'PrePnlManager.aspx?jc=" & jc & "&mode=edit&bid=' + convert(varchar(10),BriefID) + '&kid=y' end) "
        'sql &= "  Else 'BriefManagerview.aspx?mode=edit&bid=' +  	   convert(varchar(10),BriefID) + 	'&kid=y' end,  	 jc.jobCardID,     " ',
        sql &= "  Else 'PrePnlManager.aspx?jc=" & jc & "&mode=edit&bid=' +  	   convert(varchar(10),BriefID) + 	'&kid=y' end,  	 jc.jobCardID,     " ',
        ...
        sql &= "	  JobStatus = case when (JobCompleted='N' or JobCompleted is null) and JobCardNo is null then '-' 					"
        sql &= "	      when (JobCompleted='N' or JobCompleted is null) and JobCardNo is not null then 'Open' 							"
        sql &= "		     else 'Close' end,jc.INsertedOn,ProjectManagerID," & getLoggedUserID() & " as pID  from  APEX_JobCard as jc   	  							 "
        sql &= "			   Inner join APEX_Notifications as nt on jc.RefBriefID=nt.AssociateID   	  							   "
        sql &= "			   left outer join APEX_ActivityType  as At on Jc.PrimaryActivityID=At.ProjectTypeID   	  							    "
        sql &= "			   join dbo.APEX_Recieptents as rec on nt.notificationID=rec.refnotificationID   	  							   "
        sql &= "			   inner join APEX_Brief as b on jc.RefBriefID = b.BriefID   	  							   "
        ...
        'sql &= "			   Left Join Apex_task T on JC.jobcardID=T.RefjobcardID	 							   "
        sql &= "			   Left join Apex_taskAccount TA on JC.jobcardID=TA.RefjobcardID     							  "
        '(ProjectManagerID in(" & capex.getuserdetailsID(getLoggedUserID()) & ") or ProjectManagerID= " & getLoggedUserID() & " or  TA.TL=" & getLoggedUserID() & ")  and
        sql &= "			    where   JC.jobcardID=" & jc & "   order by jc.INsertedOn desc"
        sql &= " "
        sql &= ""
        sql &= ""
        sql &= " "
    ...
    Public Function FillDetailsPM3() As String
        ...
        Dim sql As String = ""
        sql &= "	select  (Case when ta.TL=" & getLoggedUserID() & " then 'Y' else 'N' end)ISclaim,AccountID ,ta.RefjobcardID,(select isnull(jobcardno,'N') from apex_jobcard where jobcardID=ta.RefjobcardID)jobcardNo,"
        sql &= " REPLACE(REPLACE(REPLACE(Cast([Particulars] as varchar(max)), CHAR(13), ''), CHAR(10), ''), CHAR(9), '') as Particulars,"
        sql &= " REPLACE(REPLACE(REPLACE(Cast([Taskname] as varchar(max)), CHAR(13), ''), CHAR(10), ''), CHAR(9), '') as Taskname,StartDate,EndDate,REPLACE(REPLACE(REPLACE(Cast([Description] as varchar(max)), CHAR(13), ''), CHAR(10), ''), CHAR(9), '') as Description,Quantity,ta.Total,category,"
        sql &= "	(isnull(ud.firstName,'')+' '+isnull(ud.LastName,''))Name       "
        sql &= "	,Status,ta.Remarks,TaskCompleted,Workstatus,ta.insertedby,(select isnull(IsPostPnL,'N')IsPostPnL  from apex_jobcard where jobcardID=ta.RefjobcardID)IsPostPnL   "
        sql &= "  ,(select case when DATEDIFF(day,ActivityEndDate,getdate()) > 21 then 'Y' else 'N' end  from apex_jobcard where jobcardID=ta.RefjobcardID)Isclaimedactive"
        ...
        sql &= "	where CM.refTaskID=TA.accountID and isapproved='Y'),0)Claimed,EAE.AdvanceAmtReq as Advanceamtrequest,EA.BranchHeadApproval,EA.FinaceApproval,VAE.AdvanceAmtReq as VAdvanceamtrequest,VA.BranchHeadApproval as VBranchHeadApproval,VA.FinaceApproval as VFinaceApproval from APEX_TaskAccount ta"
        sql &= "	left outer join APEX_UsersDetails as ud on  ud.UserDetailsID=ta.TL"
        sql &= " left join Apex_EmployeeAdvanceExpenses EAE on ta.AccountID=EAE.ReftaskID left join Apex_EmployeesAdvence EA On EAE.RefEmpAdvanceID=EA.ID"
        sql &= " left join Apex_VendorAdvanceExpenses VAE on ta.AccountID=VAE.ReftaskID left join Apex_VendorAdvence VA On VAE.RefEmpAdvanceID=VA.ID"
        sql &= ""
        sql &= "	 where ta.RefjobcardID=" & Request.QueryString("jid").ToString() & " and (ta.InsertedBy=" & getLoggedUserID() & " or ta.TL=" & getLoggedUserID() & ") order by Category"
        sql &= ""
    ...
    Private Function FillDetailsPM4() As String
        ...
        sql &= " category categoryTask,REPLACE(REPLACE(REPLACE(Cast(t.Description as varchar(max)), CHAR(13), ''), CHAR(10), ''), CHAR(9), '') as Description,  TaskCompleted =case when ta.TaskCompleted='Y' then 'Completed' else 'Pending' end  ,"
        sql &= " isnull(ud.firstName,'')+' '+isnull(ud.LastName,'') as Name,	   (select (isnull(firstName,'')+' '+isnull(LastName,'')) "
        sql &= " from APEX_UsersDetails where UserDetailsID=ta.insertedby)AssignFrom,Total as Expence,REPLACE(REPLACE(REPLACE(Cast([particulars] as varchar(max)), CHAR(13), ''), CHAR(10), ''), CHAR(9), '') as particulars,AccountID,status,"
        sql &= " REPLACE(REPLACE(REPLACE(Cast(ta.Remarks as varchar(max)), CHAR(13), ''), CHAR(10), ''), CHAR(9), '') as Remarks,ta.insertedby,ClaimStatus = case when IsApproved is null then 'Pending' when  IsApproved ='Y' then 'Approved' 	"
        sql &= " when  IsApproved ='C' then 'Canceled' when  IsApproved ='P' then 'Pending approval from KAM' else  'Rejected' End,isnull(sum(cl.Amount),'0.00') as Amount,ClaimMasterID,sum(RequestedAmt)RequestedAmt"
        ...
        Else
            sql &= "	  and (ta.insertedby =" & getLoggedUserID() & "  or  ta.insertedby in(" & capex.getuserdetailsID(getLoggedUserID()) & ") or ta.TL in(" & capex.getuserdetailsID(getLoggedUserID()) & ") or ta.TL=" & getLoggedUserID() & ")"
        End If
    ...
    Private Function fillDetailsKAM1() As String
        ...
        Dim sql As String = "select NotificationTitle,REPLACE(REPLACE(REPLACE(Cast(NotificationMessage as varchar(max)), CHAR(13), ''), CHAR(10), ''), CHAR(9), '') as NotificationMessage,convert(varchar(10),StartDate,105) as NotificationDate,Status = case when IsExecuted='Y' then 'Closed' else 'Open' End"
        ...
        sql &= " when type = 17 and IsExecuted='N'  then 'RembursementClaimApprovalview.aspx?jc=' + convert(varchar(50),(select top 1 jobcardno from Apex_claimMaster where claimmasterID=AssociateID)) + '&type=Task&nid=' + convert(varchar(10),NotificationID) + '&aid=' + convert(varchar(10),AssociateID) + '&mode=rej'  "
        ...
        sql &= " when type = 32 then link + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 33 then link + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 34 then link + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 35 then link + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 36 then link + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 37 then link + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 38 then 'openEstimate.aspx?mode=generated&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) +'&Appro=Y'"
        sql &= " when type = 39 then 'openEstimate.aspx?mode=generated&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 40 then 'openEstimate.aspx?mode=generated&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 41 then 'PrepnlapprovalfromBH.aspx?mode=generated&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID)  +'&Appro=Y#prepnl' "
        sql &= " when type = 42 then 'OpenPrePnlManager.aspx?mode=edit&kid=y&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 43 then 'OpenPrePnlManager.aspx?mode=edit&kid=y&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 44 then 'RembursementClaimApproval.aspx?jc=' + convert(varchar(50),(select top 1 jobcardno from Apex_claimMaster where claimmasterID=AssociateID)) + '&type=Task&nid=' + convert(varchar(10),NotificationID) + '&aid=' + convert(varchar(10),AssociateID)  "
        sql &= " when type = 45 then 'Estimate_VS_Actuals.aspx?jid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID)  "
        'sql &= " else '' end"
        sql &= " end from APEX_Recieptents as rc"
    ...
    Private Function filltaskdetail(ByVal jc As Integer) As String
        ...
        sql &= ""
        sql &= ""
        sql &= " select (select count(AccountID) from APEX_TaskAccount where RefJobcardID=" & jc & " and (insertedby in(select userdetailsID from Apex_usersdetails where superiorID=" & getLoggedUserID() & " or userdetailsID=" & getLoggedUserID() & ")  or TL=" & getLoggedUserID() & ")) as total"
        sql &= ",(select count(AccountID) from APEX_TaskAccount where RefJobcardID=" & jc & " and TL=" & getLoggedUserID() & ") as Ytotal"
        sql &= " ,(select count(AccountID) from APEX_TaskAccount where RefJobcardID=" & jc & " and (insertedby in(select userdetailsID from Apex_usersdetails where superiorID=" & getLoggedUserID() & " or userdetailsID=" & getLoggedUserID() & ") or TL=" & getLoggedUserID() & ")  and [status] <> 'Completed' )Running"
        sql &= " ,(select count(AccountID) from APEX_TaskAccount where RefJobcardID=" & jc & " and TL=" & getLoggedUserID() & " and [status] <> 'Completed' )YRunning"
        sql &= " , (select count(AccountID) from APEX_TaskAccount where RefJobcardID=" & jc & " and (insertedby in(select userdetailsID from Apex_usersdetails where superiorID=" & getLoggedUserID() & " or userdetailsID=" & getLoggedUserID() & ") or TL=" & getLoggedUserID() & ")  and [status] = 'Completed' ) Complete"
        sql &= " ,(select count(AccountID) from APEX_TaskAccount where RefJobcardID=" & jc & " and TL=" & getLoggedUserID() & " and [status] = 'Completed' )YComplete"
        sql &= ""
        sql &= ""
    ...
    Private Function fillprepnlsummary(ByVal jc As Integer) As String
        ...
        sql &= "select category,isnull(task,0)task,isnull(Claimed,0)Claimed,Categoryb,isnull(preeventTotal,0)Budget from ("
    ...
    Private Function fillJCsummary(ByVal jc As Integer) As String
        Dim jsonstring As String = ""
        Dim capex As New clsApex
        Dim briefid As String = capex.GetBriefIDByJobCardID(jc)
        Dim sql As String = ""
        ...
        sql &= "Select (Select Sum(preeventcost)pre from [APEX_prePnLCost] where refbriefID in (" & briefid & "))Prepnl, "
        sql &= "  isnull((select Sum(posteventcost) from [dbo].[APEX_tempPostPnLCost] where refbriefid=" & briefid & "),0.00)PostPnl"
    ...
    Private Function fillJCsummaryForEstimate(ByVal jc As Integer) As String
        Dim jsonstring As String = ""
        Dim capex As New clsApex
        Dim briefid As String = capex.GetBriefIDByJobCardID(jc)
        Dim sql As String = ""
        ...
        sql &= ""
        sql &= " Select(Select top 1 EstimatedTotal from Apex_Estimate where refbriefID in (" & briefid & "))Estimate, "
        sql &= " isnull((Select top 1 total from Apex_TempEstimate where refbriefID in (" & briefid & ")),0.00)FinalEstimate"

        sql &= ",(Select Sum(preeventcost)pre from [APEX_prePnLCost] where refbriefID in (" & briefid & "))Prepnl, "
        sql &= "  isnull((select Sum(posteventcost) from [dbo].[APEX_tempPostPnLCost] where refbriefid=" & briefid & "),0.00)PostPnl"
        sql &= ""
    ...
    Private Function filltotalbudget(ByVal jc As Integer) As String
        ...
        sql &= "	,isnull((select sum(amount) from [dbo].[APEX_ClaimTransaction] CT"
        sql &= "		inner join APEX_ClaimMaster CM on CM.claimMasterID=CT.RefClaimID"
        sql &= "		inner join Apex_usersdetails UT on CM.SubmittedUserID= UT.UserdetailsID"
        sql &= "		where jobcardNo=" & jc & " and isapproved='Y'),'0.00')Claimed	"
        sql &= " "
        sql &= " "
    ...
    Public Function FillNotification() As String
        Dim jsonstring As String = ""
        Dim sql As String = "Select top 1 NotificationID,NotificationTitle,dbo.udf_StripHTML(replace(NotificationMessage,'<br />','\n'))NotificationMessage, "
        ...
        sql &= " when type = 17 then 'RembursementClaimApprovalview.aspx?jc=' + convert(varchar(50),(select top 1 jobcardno from Apex_claimMaster where claimmasterID=AssociateID)) + '&type=Task&nid=' + convert(varchar(10),NotificationID) + '&aid=' + convert(varchar(10),AssociateID) + '&mode=rej' "
        ...
        sql &= " when type = 40 then 'openEstimate.aspx?mode=generated&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 41 then 'PrepnlapprovalfromBH.aspx?mode=generated&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID)  +'&Appro=Y#prepnl' "
        'sql &= " when type = 42 then 'PrePnlApproval.aspx?mode=edit&kid=y&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        'sql &= " when type = 43 then 'PrePnlApproval.aspx?mode=edit&kid=y&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 42 then 'OpenPrePnlManager.aspx?mode=edit&kid=y&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 43 then 'OpenPrePnlManager.aspx?mode=edit&kid=y&bid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID) "
        sql &= " when type = 44 then 'RembursementClaimApproval.aspx?jc=' + convert(varchar(50),(select top 1 jobcardno from Apex_claimMaster where claimmasterID=AssociateID)) + '&type=Task&nid=' + convert(varchar(10),NotificationID) + '&aid=' + convert(varchar(10),AssociateID)  "
        sql &= " when type = 45 then 'Estimate_VS_Actuals.aspx?jid=' + convert(varchar(10),AssociateID) + '&nid=' + convert(varchar(10),NotificationID)  "
        sql &= " end ,rp.InsertedOn from APEX_Notifications as nt Inner join APEX_Recieptents as rp on nt.NotificationID = rp.RefNotificationID where rp.UserID=" & getLoggedUserID() & " and (IsExecuted <> 'Y' or  IsSeen<>'Y')"
        ' and IsExecuted <> 'Y' and  IsSeen<>'Y' nt.InsertedOn Order By  nt.notificationID  desc
        sql &= " and deskview='N'"
        sql &= " Order By RecieptentID desc"
        Dim ds As New DataSet
        Dim dt As New DataTable
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            For i As Integer = 0 To dt.Rows.Count - 1
                sqlstr &= "Update APEX_Notifications set deskview='Y' where NotificationID='" & dt.Rows(i)("NotificationID") & "';"
            Next
            If sqlstr <> "" Then
