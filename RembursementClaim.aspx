                                                ...
                                                <Columns>
                                                   ...
                                                   <asp:TemplateField HeaderText="No of Days">
                                                    ...
                                                    <asp:TemplateField>
                                                     ...
                                                     <asp:TemplateField HeaderText="">
                                                        <HeaderTemplate><span style="color: red;">*</span>Basic</HeaderTemplate>
                                                        <ItemTemplate>
                                                            <asp:TextBox ID="txtbasic" runat="server" MaxLength="8" onkeyup="javascript:caltotal(this)" CssClass="right_textbox" Width="50"></asp:TextBox>
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
                                                     <asp:TemplateField HeaderText="">
                                                        <HeaderTemplate><%--<span style="color: red;">*</span>--%>Taxes <br />(Service Tax[Basic + SBC]<br /> + CST + VAT)</HeaderTemplate>
                                                        <ItemTemplate>
                                                            <asp:TextBox ID="txttaxes" runat="server" MaxLength="8" Text="0" onkeyup="javascript:caltotal(this)" CssClass="right_textbox" Width="80"></asp:TextBox>
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
                                                    <asp:TemplateField HeaderText="">
                                                        <HeaderTemplate><span style="color: red;">*</span>Amount</HeaderTemplate>
                                                        <ItemTemplate>
                                                            <asp:TextBox ID="txtAmount" runat="server" Enabled="false"   MaxLength="8" onkeyup="javascript:caltotal(this)" CssClass="right_textbox" Width="80"></asp:TextBox>
                                                            <asp:HiddenField ID="hdnamount" Value="0" runat="server" />
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
                                                    <asp:TemplateField HeaderText="Submission Type">
                                                        <ItemTemplate>
                                                            <asp:DropDownList ID="ddlsubmissionType" runat="server" CssClass="textbox" Width="120px">
                                                                <asp:ListItem>Claim</asp:ListItem>
                                                                <asp:ListItem>Employee advance</asp:ListItem>
                                                                <asp:ListItem>Vendor advance</asp:ListItem>
                                                            </asp:DropDownList>
                                                        </ItemTemplate>
                                                    </asp:TemplateField>
...
<asp:Content ID="Content3" ContentPlaceHolderID="cpFooter" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function caltotal(obj) {
            ...
            if ($('input[id$=txtbasic' + postfix + ']').val() != "") {

                var numberRegex = /^\d{1,10}(\.\d{0,2})?$/;
                if (numberRegex.test($('input[id$=txtbasic' + postfix + ']').val())) {
                    val1 = true

                }
                else {
                    alert('Enter numeric data only');
                    $('input[id$=txtbasic' + postfix + ']').val("");

                }
            }
            else {
                $('input[id$=txtbasic' + postfix + ']').val(0);
            }

            if ($('input[id$=txttaxes' + postfix + ']').val() != "") {

                var numberRegex = /^\d{1,10}(\.\d{0,2})?$/;
                if (numberRegex.test($('input[id$=txttaxes' + postfix + ']').val())) {
                    val2 = true;
                }
                else {
                    alert('Enter numeric data only');
                    $('input[id$=txttaxes' + postfix + ']').val("");
                }
            }
            else {
                $('input[id$=txttaxes' + postfix + ']').val(0);
            }
           
            if (val1 == true && val2 == true ) {

                var basic = parseFloat($('input[id$=txtbasic' + postfix + ']').val());
                var tax = parseFloat($('input[id$=txttaxes' + postfix + ']').val());
                //var total = parseFloat($('input[id$=txtAmount' + postfix + ']').val());

                var total = (basic + tax).toFixed(2);
               //alert(total);
                //  alert(total);
                //$('span[id$=txtAmount' + postfix + ']').val(total);
                //$('span[id$=gv_txtPreEventCost' + postfix + ']').text(total);

                $('input[id$=txtAmount' + postfix + ']').val(total);
                $('input[id$=hdnamount' + postfix + ']').val(total);
