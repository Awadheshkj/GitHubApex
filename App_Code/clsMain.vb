...
#End Region
    ...
    Public Shared Function sendMailforEmployeeAdvance(ByVal Subject As String, ByVal mailBody As String, ByVal PlainText As String, ByVal ToAddress As String, ByVal KamCCAddress As String, ByVal PMCCAddress As String, Optional ByVal FromAddress As String = "alert@kestoneapps.com", Optional ByVal FromName As String = "Kestone") As Boolean
        Try
            Dim mailOB As New MailMessage

            mailOB.IsBodyHtml = True
            mailOB.From = New MailAddress(FromAddress, FromName)
            Dim taddressarr = ToAddress.Split("|")

            For i As Integer = 0 To taddressarr.Length - 1
                mailOB.To.Add(taddressarr(i))
            Next

            'mailOB.To.Add(New MailAddress(ToAddress))

            If KamCCAddress.Trim <> "" Then
                'mailOB.CC.Add(New MailAddress(CCAddress))
                mailOB.CC.Add(KamCCAddress)
            End If
            If PMCCAddress.Trim <> "" Then
                'mailOB.CC.Add(New MailAddress(CCAddress))
                mailOB.CC.Add(PMCCAddress)
            End If
