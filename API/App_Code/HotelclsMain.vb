...
#Region "  Authentication  "
    ...
    Public Shared Function requestAccess(ByVal Username As String, ByVal Password As String, Optional ByVal Remember As Boolean = False) As _ReturnCodes
        ...
        'SQL = "select RefUserDetailsID,loginID,username,password,DisplayName,IsLoginAllowed,isactive,isdeleted,EmailID from [dbo].[APEX_Login] where BINARY_CHECKSUM(" & config.Username & ") = BINARY_CHECKSUM('" & Username.Replace("'", "''") & "')"
        SQL += ""
        SQL += " Select * from (select ContactID as RefUserDetailsID,contactofficialEmailID as username,password,"
        SQL += " ContactName as DisplayName,IsLoginAllowed,ISACtive,ISDeleted,contactofficialEmailID as emailID"
        SQL += " from [dbo].[APEX_ClientContacts]"
        SQL += " union all"
        SQL += " select RefUserDetailsID,username,password,DisplayName,IsLoginAllowed,isactive,isdeleted,EmailID "
        SQL += " from [dbo].[APEX_Login]) A"
        SQL += " where BINARY_CHECKSUM(" & config.Username & ") = BINARY_CHECKSUM('" & Username.Replace("'", "''") & "')"
        SQL += ""
        SQL += ""
        SQL += ""
