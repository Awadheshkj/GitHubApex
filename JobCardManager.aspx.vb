...
Partial Class JobCardManager
        ...
        Try
            ...
            If Not Page.IsPostBack() Then
                    ...
                    If Request.QueryString("jid") <> Nothing Then
                        If Request.QueryString("jid").ToString() <> "" Then
                            ...
                            If checkforAccountdepartment() = True Then
                                ...
                                TxtapprovlAmount.Enabled = False
                ...
                Else
                    CallDivError()
                End If
                hdnapprovalamount.Value = TxtapprovlAmount.Text
            End If
        ...
        Try
            ...
            If CheckJobCardExists() = False Then
                ...
                If Convert.ToDateTime(txtProjectStartDate.Text, culture) < Convert.ToDateTime(txtProjectEndDate.Text, culture) Then
                    ...
                    If hdnapprovalamount.Value <> TxtapprovlAmount.Text Then
                        Label1.Text = "Client approval verification request has been sent to Finance Team."
                        InsertJCHistory(hdnJobCardID.Value, lblJobCode.Text, TxtapprovlAmount.Text, Path)

                        sql &= ", JCRequestAmount=" & Clean(TxtapprovlAmount.Text)
                        sql &= ",LastRequestedOn=getdate()"
            ...
            Else
                ...
                End If
            End If
            ClientScript.RegisterStartupScript(Page.GetType(), "Anchor", "location.hash = '#divError';", True)
        Catch ex As Exception
            SendExMail(ex.Message, ex.ToString())
        End Try
    End Sub
    ...
    Protected Sub btnverify_Click(sender As Object, e As EventArgs) Handles btnverify.Click
        If txtJobCode.Text.ToString.Substring(0, 4) = "KIMS" Then
            lblError.Text = "Please assign Actual JC against temprary JC"
            divError.Visible = True
            txtJobCode.Focus()
            Exit Sub
        Else
            divError.Visible = False
            If btnverify.Text = "Approve" Then
            ...
            If messageshow = "hide" Then
                'If txtJobCode.Enabled = False Then
    ...
    End Sub
    Public Sub UpdateFMRecieptentExecutedall()
        Dim i As Integer = 0
        Dim sql As String = "Update APEX_Recieptents set IsExecuted='Y' where RefNotificationID in(select NotificationID from APEX_Notifications where type=28 and AssociateID='" & hdnJobCardID.Value & "')"
        If ExecuteNonQuery(sql) > 0 Then
            i = i + 1
        Else
            i = i + 2
    ...
    Public Sub SendForclientApprovedToFM(ByVal type As String)
        Try
            ...
            sql &= " INSERT INTO APEX_Notifications"
            sql &= "(NotificationTitle"
            sql &= ",NotificationMessage"
            sql &= ",Severity"
            sql &= ",Type"
    ...
    Private Sub verifiyclientapproval(jcID As String, ByVal ISclientapprovalverified As String)
        ...
        Dim sql As String = ""
        If ISclientapprovalverified = "Y" Then
            status = "Approved"

            sql &= " update Apex_jobcard set ISclientapprovalverified='" & ISclientapprovalverified & "',fmverifyOn=getdate(),JcApprovalAmount='" & TxtapprovlAmount.Text & "',FinanceApprovalOn=getdate() where jobcardID='" & jcID & "'"

            sql &= ";"
        ElseIf ISclientapprovalverified = "N" Then
            status = "Reject"
        End If


        sql &= " update Apex_JobCardHistory set ApprovedAmount='" & TxtapprovlAmount.Text & "', approvedOn=getdate(),approvedBy='" & getLoggedUserID() & "',Status='" & status & "' where refjobcardID='" & hdnJobCardID.Value & "' and HID=" & hdnhistoryID.Value
        ExecuteNonQuery(sql)
    ...
    Private Function InsertJCHistory(ByVal refjobcardID As Integer, JobcardNo As String, RequestedAmount As Decimal, ApprovalMail As String) As Boolean

        Try
            Dim sql1 As String = ""
            sql1 &= " update Apex_JobCardHistory set ApprovedAmount=0, approvedOn=getdate(),approvedBy='" & getLoggedUserID() & "',Status='Reject' where refjobcardID='" & hdnJobCardID.Value & "' and ApprovedAmount=0 "
            'HID=" & hdnhistoryID.Value
            ExecuteNonQuery(sql1)

            Dim sql As String = ""
            sql &= "    INSERT INTO [dbo].[Apex_JobCardHistory]"
            sql &= "           ([RefJobcardID]"
            sql &= "           ,[JobcardNo]"
            sql &= "           ,[RequestedAmount]"
            sql &= "           ,[ApprovedAmount]"
    ...
    Private Sub getjobcardHistory()
        Dim sql As String = ""

        sql &= "	select HID, RefJobcardID, JobcardNo, RequestedAmount, ApprovedAmount, RequestedOn, ApprovedOn,"
        sql &= "	ApprovalMail,Status,"
        sql &= "	(select (Firstname + ' ' + Isnull(LastName,''))EmpName from [dbo].[APEX_UsersDetails] where userdetailsID=ApprovedBy)"
        sql &= "	ApprovedBy	from Apex_JobCardHistory where refjobcardID='" & hdnJobCardID.Value & "' order by InsertedOn desc"
        sql &= ""
        ...
        ds = ExecuteDataSet(sql)
        If ds.Tables(0).Rows.Count > 0 Then
            TxtapprovlAmount.Text = ds.Tables(0).Rows(0)("RequestedAmount")
            hdnhistoryID.Value = ds.Tables(0).Rows(0)("HID")
            gridjobcardhistory.DataSource = ds
            gridjobcardhistory.DataBind()
        End If


    End Sub
    Protected Sub lblfulpl_Mail1_Click(sender As Object, e As EventArgs)
        Dim gvRow As GridViewRow = CType(CType(sender, Control).Parent.Parent,
                                        GridViewRow)
        Dim index As Integer = gvRow.RowIndex


        Dim hdnpath As String =
              CType(gvRow.FindControl("hdnpath"), HiddenField).Value
        ...
        'Dim str As String = hdnlblfulpl_Mail.Value
        Response.AppendHeader("Content-Disposition", "attachment; filename=" & Path.GetFileName(hdnpath) & "")
        Response.TransmitFile("~/" & hdnpath.ToString())
        'Response.TransmitFile("~/Collateral/Jobcard/65K Fund 2015422000.msg")
        Response.End()
    End Sub
