...
Partial Class RembursementClaimApproval
    ...
    Private Sub fillDetails()
        ...
        sql &= " where ClaimMasterID in (" & hdnID.Value & ")"
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            For i As Integer = 0 To ds.Tables(0).Rows.Count() - 1
                If i = 0 Then
                    sheetnumber &= ds.Tables(0).Rows(i)("ClaimMasterID").ToString()
                Else
                    sheetnumber &= "," & ds.Tables(0).Rows(i)("ClaimMasterID").ToString()
                End If
            Next
            lblclaimID.Text = "(" & capex1.GetJobCardNoByJobCardID(ds.Tables(0).Rows(0)("jobcardno")) & "/" & sheetnumber & ")"
            ...
            'If lblclaimstatus.Text = "Approved" Then
        ...
        If hdnJobCardID.Value <> "" Then
            ...
            Dim sql2 As String = "Select JobCardNo,jobcardname,(select ProjectType from [dbo].[APEX_ActivityType] where projectTypeID=primaryActivityID)ProjectType  from APEX_JobCard where JobCardID = " & hdnJobCardID.Value
            ...
            If ds2.Tables(0).Rows.Count > 0 Then
                ...
                lblprimaryActivites.Text = ds2.Tables(0).Rows(0)("ProjectType")
    ...
    Private Sub fillClaims()
        ...
        'sql += "    jobcardid=" & hdnJobCardID.Value & "))Budge  ,"
        ...
        'sql += "    where RefjobcardID=" & hdnJobCardID.Value & "  )A "
        ...
        'sql += "    where cm.RefTaskID =" & hdnTaskID.Value & " and jobcardNo=" & hdnJobCardID.Value & " and claimmasterID In (" & hdnID.Value & "))B "
        'sql += "	on A.AccountID=B.RefTaskID"
        'sql += ""
        'sql += ""

        sql.Append("	select (select categoryName from Apex_ClaimsCategory where ID=CT.RefExpe_category)Exp_Type,Date,Description,")
        sql.Append("	VenderName,Amount as ApprovedAmt,Remarks,ExpenseperiodFrom,ExpensePeriodTo,Expenseplace,submissionType,Basic,taxes,convert(varchar(50),isnull(approvedOn,''),106)approvedOn")
        sql.Append("	from [dbo].[APEX_ClaimTransaction] CT")
        sql.Append("	inner join APEX_ClaimMaster CM on CM.claimMasterID=CT.RefClaimID")
        sql.Append("	inner join Apex_usersdetails UT on CM.SubmittedUserID= UT.UserdetailsID")
        sql.Append("	where jobcardNo=" & hdnJobCardID.Value & " and refclaimID in(" & hdnID.Value & ") ")
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            lblapprovaldate.Text = ds.Tables(0).Rows(0)("approvedOn")
    ...
    Private Sub CheckAccountBalance()
        ...
        sql += "  from  Apex_taskAccount where accountID in(" & hdnTaskID.Value & ")"
        sql += ""
        sql += ""
        sql += ""
        sql += ""
    ...
    Private Sub fillalldetails()
        Try
            ...
            sql.Append("select")
            sql.Append("( select ")
            sql.Append("(select isnull(sum(PreEventcost),0) as PreEventQuote from APEX_PrePnLcost where  refbriefID=")
            'category=Apex_taskAccount.category and
            sql.Append("(select refbriefID from apex_jobcard where jobcardid=" & hdnJobCardID.Value & "))Total")
            ...
            sql.Append(" where SubmittedUserID=" & ID.ToString() & " and jobcardno=" & hdnJobCardID.Value & "),'0.00') RequestedT,")
            sql.Append(" Isnull(  (select sum(amount) from APEX_ClaimMaster CM ")
            sql.Append(" inner join APEX_ClaimTransaction on RefclaimID=CM.claimmasterID")
            sql.Append(" where SubmittedUserID=" & ID.ToString() & " and jobcardno=" & hdnJobCardID.Value & " and Isapproved='Y'),'0.00') ApprovedB,")
            sql.Append(" Isnull(  (select sum(amount) from APEX_ClaimMaster CM ")
            sql.Append(" inner join APEX_ClaimTransaction on RefclaimID=CM.claimmasterID")
            sql.Append(" where SubmittedUserID=" & ID.ToString() & " and jobcardno=" & hdnJobCardID.Value & " and IsApproved Is Null),'0.00')PendingB")
    ...
    Private Sub bindapprovalstatus()
        Dim sql As String = ""
        sql &= " select (Case when IsApproved='R' then 'Rejected' when IsApproved='Y' then 'Approved' else 'Pending' end)PmStatus,"
        sql &= " convert(varchar(30),ApprovedOn,106)pmApprovalON,(Select firstname from  Apex_usersdetails where userdetailsID=Approvedby)PMname"
        sql &= " ,(Case when KamApprovalStatus='R' then 'Rejected' when KamApprovalStatus='Y' then 'Approved' else 'Pending' end)KAMStatus,"
        sql &= " isnull(convert(varchar(30),KamApprovalOn,106),'N/A')KamApprovalOn"
        sql &= " ,isnull((Select firstname from  Apex_usersdetails where userdetailsID=KamApprovedby),'N/A')Kamname"
        sql &= " from APEX_ClaimMaster where claimmasterID in(" & hdnID.Value & ")"
        sql &= ""

        Dim ds As New DataSet
        ds = ExecuteDataSet(sql)
        If ds.Tables(0).Rows.Count > 0 Then
            gdvstatusdetails.DataSource = ds
            gdvstatusdetails.DataBind()
            lblclaimstatus.Text = ds.Tables(0).Rows(0)("KAMStatus")
            If lblclaimstatus.Text = "Approved" Then
                pnlprint.Visible = True
            Else
                pnlprint.Visible = False
